  ...
  async onload() {
    ...
    try {
      ...
      try {
        ...
        // before provider services resolve storage/CLI paths, since they read both
        // the global (~/.claude) and vault-level (.claude) paths. The global home
        // and the in-vault config directory are configured independently so a
        // custom global home can coexist with `.claude/`.
        {
